...
import { usePointerInteractions } from "~/lib";
import { clsx } from "..";
...
export function IconButton({
  ...
  return (
    <button
      {...props}
      className={clsx(
        `rounded-sm px-1 py-[1px]`,
        active ? "text-primary-900" : "",
        props.disabled
          ? `bg-primary-50 text-primary-400`
          : active
          ? `cursor-pointer bg-primary-100 text-primary-900 hover:bg-primary-200 hover:text-primary-900`
          : `cursor-pointer bg-primary-100 text-primary-500 hover:bg-primary-200 hover:text-primary-900`,
        className,
      )}
    >
      {IconLibrary[icon]}
...
function FloatingMenu({ editor }: FloatingMenuComponentProps) {
  ...
  return (
    <div className="flex items-center justify-between gap-1 rounded border border-primary-300 bg-primary-100 p-1">
      <IconButton
        icon="bold"
