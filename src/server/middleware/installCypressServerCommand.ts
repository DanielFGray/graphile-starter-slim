...
export async function installCypressServerCommand(app: Express) {
  ...
  const handleCypressServerCommand: RequestHandler = async (req, res, next) => {
     ...
     */
    if (!safeToRun) {
      console.error("/cypressServerCommand denied because ENABLE_CYPRESS_COMMANDS is not set.");
  ...
  app.get("/cypressServerCommand", urlencoded({ extended: false }), handleCypressServerCommand);
}
...
async function runCommand(
  ...
  if (command === "clearTestUsers") {
    await rootPgPool.query("delete from app_public.users where username like 'testuser%'");
    return { success: true };
  } else if (command === "clearTestOrganizations") {
    await rootPgPool.query("delete from app_public.organizations where slug like 'test%'");
    return { success: true };
    ...
    await rootPgPool.query("update app_public.users SET is_verified = TRUE where username = $1", [
      username,
    ]);
    return { success: true };
  } else {
