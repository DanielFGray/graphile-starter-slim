...
const PassportLoginPlugin = makeExtendSchemaPlugin(build => {
  ...
  const currentUserIdResource = build.input.pgRegistry.pgResources.current_user_id;
  if (!userResource || !currentUserIdResource) {
    throw new Error("Couldn't find either the 'users' or 'current_user_id' source");
  }
  const plans: Plans = {
    RegisterPayload: {
      user($obj) {
    ...
    LoginPayload: {
      user() {
        const $userId = currentUserIdResource.execute() as PgClassExpressionStep<any, any>;
        return userResource.get({ id: $userId });
      },
    },
  };
